...
def find_ball(opencv_image, debug=False):
	...
	img_noise_out = cv2.medianBlur(opencv_image, 3)
	edge = cv2.Canny(img_noise_out, 200, 200)
	ball1 = cv2.HoughCircles(img_noise_out,cv2.HOUGH_GRADIENT,1,500,200,200,5,10) # 200, 200, 5, 20, (250,400,5,20)
	ball2 = cv2.HoughCircles(edge,cv2.HOUGH_GRADIENT,1,500,200,200,5,10) # 200, 200, 5, 20, (250,400,5,20)

	if (ball1.shape == (1, 1, 3)) and (ball2.shape == (1, 1, 3)):

		x_dist = abs(ball1[0, 0, 0] - ball2[0, 0, 0])
		y_dist = abs(ball1[0, 0, 1] - ball2[0, 0, 1])
		r_dist = abs(ball1[0, 0, 2] - ball2[0, 0, 2])

		if (x_dist < 20) and (y_dist < 20) and (r_dist < 20):
			ball = ball1
		#			print("Found!")
		else:
			ball = [[[0, 0, 0]]]
